  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      <div className="page-head" style={{ justifyContent: 'space-between' }}>
        <div className="row"><Leaf size={24} /><H1 style={{ color: 'var(--text-pri)' }}>MongoDB Assistant</H1></div>
        <select className="mono" value={ctx ? clusterKey(ctx) : ''} onChange={e => setCtx(clusters.find(c => clusterKey(c) === e.target.value) || null)}
          style={{ background: 'var(--bg-card)', color: 'var(--text-pri)', border: '1px solid var(--border-accent)', borderRadius: 20, padding: '6px 14px', fontSize: 12 }}>
      ...
      {config.mongodb && (
        <div className="row" style={{ marginBottom: 12, gap: 8 }}>
          <select className="mono" value={convId || ''} onChange={e => openConversation(e.target.value)}
            style={{ background: 'var(--bg-card)', color: 'var(--text-pri)', border: '1px solid var(--border-subtle)', borderRadius: 6, padding: '6px 10px', fontSize: 12, maxWidth: 420 }}>
      ...
      {msgs.length === 0 && (
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 10 }}>Comece com uma pergunta:</div>
